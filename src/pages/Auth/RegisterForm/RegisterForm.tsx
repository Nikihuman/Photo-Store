...
export function RegisterForm({ className, ...props }: RegisterFormProps) {
	...
	const error: SubmitErrorHandler<IRegisterForm> = data => {
		dispatch(userActions.clearErrorsMessages());
		dispatch(
			userActions.addRegisterErrorsMessages({
		...
		>
			...
			<Input
				...
				{...register('name', {
					required: {
						value: true,
						message:
                     'The field \'name\' must be a string, and the length of the \'name\' field must be 2 or more characters'
					}
				})}
			/>
			<div className={cn(styles.input_password_wrapper)}>
				...
				<Button
					...
					className={cn(styles.change_password_visibility)}
				>
