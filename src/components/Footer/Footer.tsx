...
export function Footer ({className, pathname, ...props}: FooterProps){
	...
	const changePage = useCallback((options: 'increase' |'decrease') => {
	...
	return (
		<footer className={cn(className, styles.footer)} {...props}>
			{pageState <= 1 ? null : 
				<Link to={`${pathname.slice(0, pathname.search(props.find_page_number_regexp))}${props.addition_to_the_route}${pageState-1}`}>
