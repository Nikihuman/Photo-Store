...
export function Sidebar({ className, ...props }: SidebarProps) {
	...
	return (
		<nav
			className={cn(className, styles.nav, { [styles['nav_hidden']]: !sideBarState })}
			{...props}
		>
			<Button
				className={cn(styles.opened_nav_button)}
				onClick={() => {
					dispatch(floatingActions.sideBarController());
				}}
			>
			...
			<div className={cn(styles.nav_link_wapper)}>
				<NavLink to="/" className={cn(styles.nav_link)}>
					<HomeIcon />
               Home
				</NavLink>
				{
					<div className={cn(styles.profile)}>
						<button
							className={cn(styles.profile_button, {
								[styles['profile_button_active']]: profileState
							})}
							onClick={() => {
								setProfileState(state => !state);
							}}
						>
							<UserIcon />
                     Profile
						...
						<div
							className={cn(styles.profile_items, {
								[styles['profile_items_opened']]: profileState
							})}
						>
							<NavLink to="/favorite" className={cn(styles.profile_items_link)}>
								<HeartIcon />
                        Favorites
							</NavLink>
						</div>
			...
			<div className={cn(styles.nav_buttons_wrapper)}>
				{jwt ? (
					<>
						<Button
							className={cn(styles.nav_button)}
							onClick={() => {
					...
					<>
						<Link to="/auth/login">
							<Button className={cn(styles.nav_button)}>Sign In</Button>
						</Link>
						<Link to="/auth/register">
							<Button className={cn(styles.nav_button)}>Sign Up</Button>
						</Link>
					</>
				)}
			</div>
