...
import { AUTH_URL, GET_FAVORITE_PHOTOS_PATH, REMOVE_PHOTO_PATH, SAVE_PHOTO_PATH } from '../helpers/API';
...
const photosSlice = createAppSlice({
	...
	reducers: (create) =>({
		getAllPhotos: create.asyncThunk<IGetFavoriteList, void, { rejectValue: IRejectValue[]}>(
			async (_args, {rejectWithValue ,...thunkApi}): Promise<IGetFavoriteList> => {
				try{
					...
					if(!user.jwt || !user.email){
						throw rejectWithValue([{field: 'authorization', error_Message: 'No access to this area'}]);
					}
					const res = await axios.post<IGetFavoriteList>(AUTH_URL+GET_FAVORITE_PHOTOS_PATH, {
						email: user.email
					}, {
						headers:{
					...
					throw rejectWithValue([{field: 'authorization', error_Message: 'Somthing went wrong'}]);
			...
			},
			{
				rejected: (state,{payload}) => {
					if(payload) {
						state.errorMessage = payload.find((el)=>el.field ==='photo')?.error_Message ?? null;
					}
		...
		savePhoto: create.asyncThunk<{message: string, photo: PhotoResult}, ISavePhotoForm, { rejectValue: IRejectValue[]}>(
			async (args, thunkApi): Promise<{message: string, photo: PhotoResult}> => {
				try{
					const {user} = thunkApi.getState() as RootState;
					if(!user.jwt || !user.email){
						throw thunkApi.rejectWithValue([{field: 'authorization', error_Message: 'No access to this area'}]);
					}
					const res = await axios.post(AUTH_URL+SAVE_PHOTO_PATH, {
						...args
					}, {
						headers:{
							Authorization: `Bearer ${user.jwt}`
		...
		removePhoto: create.
			asyncThunk<{message: string, id: number}, {id: number},	{ rejectValue: IRejectValue[]}>(
				async ({id}, thunkApi): Promise<{message: string, id: number}> => {
					try{
						...
						const res = await axios.post(AUTH_URL+REMOVE_PHOTO_PATH, {
							id
						}, {
							headers:{
								Authorization: `Bearer ${user.jwt}`
						...
						throw thunkApi.rejectWithValue([{field: 'photo', error_Message: 'Somthing went wrong'}]);
					}
