	...
	reducers: create => ({
			...
			async ({ email, password }: ILoginForm, thunkApi): Promise<ILoginResponse> => {
				try {
					...
					throw thunkApi.rejectWithValue([
						{ field: 'email', error_Message: 'Somthing went wrong' }
					]);
				}
			},
			{
				rejected: (state, { payload }) => {
					if (payload) {
						state.loginErrorMessages.email = payload.find(
							el => el.field === 'email'
						)?.error_Message;
						state.loginErrorMessages.password = payload.find(
				...
				fulfilled: (
					state,
					{ payload }: PayloadAction<{ message: string; access_token: string }>
				) => {
		...
		register: create.asyncThunk<ILoginResponse, IRegisterForm, { rejectValue: IRejectValue[] }>(
			...
			},
			{
				rejected: (state, { payload }) => {
					if (payload) {
						state.registerErrorMessages.email = payload.find(
							el => el.field === 'email'
						)?.error_Message;
						state.registerErrorMessages.password = payload.find(
							el => el.field === 'password'
						)?.error_Message;
						state.registerErrorMessages.name = payload.find(
							el => el.field === 'name'
						)?.error_Message;
					}
				},
		...
		addRegisterErrorsMessages: create.reducer<{ email: string; password: string; name: string }>(
			(state, { payload }) => {
		...
		addLoginErrorsMessages: create.reducer<{ email: string; password: string }>(
			(state, { payload }) => {
